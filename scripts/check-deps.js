...
function showSummary(outdated) {
	...
	if (important.length) {
		console.log(
			color.bold(color.yellow(`⚠ Dependências críticas (${important.length}):`))
		);
		for (const pkg of important) {
			...
			console.log(
				`  ${color.red("●")} ${color.bold(pkg)} ${color.cyan(`[${type}]`)} ${version}`
			);
		}
		console.log("");
	} else {
		console.log(
			color.green(
				"✔ Todas as dependências críticas consideradas estão atualizadas.\n"
			)
		);
	}
